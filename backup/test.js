...
async function loadInitialData() {
    try {
        ...
        if (userResponse.ok) {
            ...
            document.querySelector('.userProfile').src = userData.img_url || 'https://via.placeholder.com/50';
        }
...
function populateWasherAndLocationDropdowns() {
    ...
    locationData.forEach(location => {
        ...
        let locationText = `${location.location_name} - ${location.location_url}`;
        if (location.mobile) {
...
function renderPastBookings(bookings) {
    //const tableBody =document.getElementById("table")// document.querySelector('.bookings-body');
    const tableBody = document.getElementById('bookingsTableBody') || 
                         document.querySelector('.bookings-body');
    ...
    if (!bookings || bookings.length === 0) {
        container.innerHTML = `
            <div class="col-12 text-center py-5">
                <h4 class="mb-3">Recent Wash Services</h4>
                <div style="font-size: 3rem;">📭</div>
     ...
     * 
     *
    // Clear existing rows (except empty state row)
...
function renderPastBookings(bookings) {
   ...
   const container = document.querySelector('.pastbookings'); // Changed from getElementById to querySelector
...
function renderFeedbackMessages(feedbackList) {
    ...
    messagesContainer.innerHTML = '<h5>Your Feedback Messages</h5>';
...
function showBookingSuccess(bookingResult) {
    ...
    successModal.querySelector('p').textContent = 
        ...
        `Washer: ${details.washer}. You'll receive a confirmation shortly.`;
...
function trackBooking(bookingId) {
    ...
    createToast("Tracking feature coming soon!", "info");
}
...
function callWasher(phoneNumber) {
    ...
    createToast(`Calling washer at ${phoneNumber}`, "info");
}
...
function validateFeedbackForm() {
    ...
    if (!type) {
        createToast('Please select a feedback type', 'error');
        return false;
    }
